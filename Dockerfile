...
RUN apt update && \
  apt install -y git libffi-dev sqlite3 python2.7 python-virtualenv python3.5 python-pip python3-pip python3-virtualenv ffmpeg bash telnet supervisor gettext-base && \
  rm -rf /srv/taiko-web/* && mkdir -p /srv/taiko-web

RUN git clone https://github.com/Mik027/taiko-web.git /srv/taiko-web && \
  cd /srv/taiko-web

RUN mkdir -p /srv/taiko-web && virtualenv -p /usr/bin/python2 .venv2 && \
  pip install pymongo && \
  virtualenv -p /usr/bin/python3 .venv3 && \
  pip3 install -r /srv/taiko-web/requirements.txt 
...
ADD config.py /srv/taiko-web/config.py
